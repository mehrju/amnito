...
var tehranCollectorAreaLayer = null;
var tabrizCityAreaLayer = null;
var ahvazCityAreaLayer = null;
var esfahanCollectorAreaLayer = null;
var _IsCod = false;
var tehranPostAreas = [
    {
        Id: 4,
...
getCollectorAreaId = function (latlng) {
    var AreaId = null;
    for (var i in tehranCollectorAreaLayer) {
        var _currentLayer = tehranCollectorAreaLayer[i];
    ...
    if (!AreaId) {
        for (var i in esfahanCollectorAreaLayer) {
            var _currentLayer = esfahanCollectorAreaLayer[i];
            var inLayer = leafletPip.pointInLayer(latlng, _currentLayer, true);
            if (inLayer.length > 0) {
                var AreaId = _currentLayer.options.layerName;
                break;
...
function createPostMap(isForgein, isFromAp, IsCod) {
    _IsCod = IsCod;
    ...
    function showSearchResult(result, isForegin) {
        ...
        var country = $('#PreCountry option:selected').text();
        var State = $('#PreState option:selected').text();
        if (!isForegin) {
            ...
            success: function (data) {
                if (data.address) {
                    ...
                    if (IsForegin) {
                        ...
                        if ($('#Address1').val() != '') {
                            if (!isFromAp) {
                                canFillAddress = confirm('آیا این آدرس با آدرس  موجود در فیلد آدرس جایگزین شود؟');
                            }
                            else {
    ...
    isIntarfficZone = function (latlng) {
        var _isIntarfficZone = false;
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 'OK' && data.state) {
                    _isIntarfficZone = data.in_traffic_zone;
                }
            }
        });
        return _isIntarfficZone;
    }
    SetLocationData = function (isCod, isHeavy) {
        ...
        if ($('#isForginRequest').val() == 'true' && $('#addressType').val() != 'Sender') {
            ...
            if ($('#newOldAddress').val() == 'newAddress') {
                if (!$('#PreCountry').val() || $('#PreCountry').val() == 0) {
                    alert('انتخاب استان در کادر بالا الزامی می باشد');
                    return;
                }
                //if ($('#PreCountry').val() != '1' && (!$('#PreState').val() || $('#PreState').val() == '0' )) {
                //     alert('انتخاب استان در کادر بالا الزامی می باشد');
                //    return;
                //}
                if ($('#addressType').val() != 'Sender') {
            ...
            if (!isHeavy) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status == 'OK' && data.state) {
                            ...
                            if (State) {
                                $('#isInCityArea').val(isInCityAreaFromLayer(latlng));
                                $('#tehranAreaId').val(getTehranAreaFromLayer(latlng));
                                if (data.in_traffic_zone) {
                                    $('#trafficArea').val('true');
                                }
                                else
                                    $('#trafficArea').val('false');
                                if (State == 'تهران') {
                                    ...
                                    if (!_tehranState) {
                                        ...
                                        if (!$('#Country option:selected').text().includes(Country))
                                            ...
                                            if ($('#State option:contains(' + _tehranState + ')').val()) {
                                                $('#State').val($('#State').find("option:contains('" + _tehranState + "')").val()).trigger('change');
                                                $('#PreState').val($('#PreState').find("option:contains('" + _tehranState + "')").val()).trigger('change');
                                            }
                                        }
                                    }
                                }
                                    ...
                                    if (!$('#Country option:selected').text().includes(Country))
                                        ...
                                        if ($('#State option:contains(' + State + ')').val()) {
                                            $('#State').val($('#State').find("option:contains('" + State + "')").val()).trigger('change');
                                            $('#PreState').val($('#PreState').find("option:contains('" + State + "')").val()).trigger('change');
                                        }
                                    }
                                }
                            }
                ...
                $('#Country').val($('#PreCountry').val()).trigger('change');
                ...
                $('#_CountryDiv').show();
                $('#_StateDiv').show();
                $('#State').val($('#PreState').val()).trigger('change');
            }
        }
    }
    postLayer = AddPostLayer(map);
    cityAreaLayer = CollectorAndDistributertLayer = createCollectorAndDistributertLayer(map);
...
function createCollectorAndDistributertLayer(map) {
    ...
    //map.createPane('pane_Tehran_1');
    //map.getPane('pane_Tehran_1').style.zIndex = 409;
    //map.getPane('pane_Tehran_1').style['mix-blend-mode'] = 'normal';
    //var layer_Tehran_1 = new L.geoJson(json_Tehran_1, {
    //    attribution: '',
    //    interactive: true,
    //    dataVar: 'json_Tehran_1',
    //    layerName: 'layer_Tehran_1',
    //    pane: 'pane_Tehran_1'
    //});
    //bounds_group.addLayer(layer_Tehran_1);
    //layerArray.push(layer_Tehran_1);
    //map.addLayer(layer_Tehran_1);
    tehranCityAreaLayer = [];
    tehranCollectorAreaLayer = [];
    ahvazCityAreaLayer = [];
    tabrizCityAreaLayer = [];
    CollectorAndDistributertLayer = [];
    esfahanCollectorAreaLayer = [];
    // مناطق شهر داری تهران
    for (var i = 1; i <= 22; i++) {
        ...
        CollectorAndDistributertLayer.push(layer_tca);
        tehranCityAreaLayer.push(layer_tca);
        map.addLayer(layer_tca);
    }
    // tehran collector area
    for (var i = 1; i <= 2; i++) {
        map.createPane('pane_Tehran_' + i.toString());
        map.getPane('pane_Tehran_' + i.toString()).style.zIndex = 401;
        map.getPane('pane_Tehran_' + i.toString()).style['mix-blend-mode'] = 'normal';
        var layer_tca = new L.geoJson(eval('json_Tehran_' + i.toString()), {
            attribution: '',
            interactive: true,
            dataVar: 'json_Tehran_' + i.toString(),
            layerName: i.toString(),
            pane: 'pane_Tehran_' + i.toString()
        });
        bounds_group.addLayer(layer_tca);
        layerArray.push(layer_tca);
        tehranCollectorAreaLayer.push(layer_tca);
        CollectorAndDistributertLayer.push(layer_tca);
        map.addLayer(layer_tca);
    }
    // esfahan collector area
    for (var i = 1; i <= 2; i++) {
        map.createPane('pane_Esfahan_' + i.toString());
        map.getPane('pane_Esfahan_' + i.toString()).style.zIndex = 401;
        map.getPane('pane_Esfahan_' + i.toString()).style['mix-blend-mode'] = 'normal';
        var layer_tca = new L.geoJson(eval('json_Esfahan_' + i.toString()), {
            attribution: '',
            interactive: true,
            dataVar: 'json_Esfahan_' + i.toString(),
            layerName: i.toString(),
            pane: 'pane_Esfahan_' + i.toString()
        });
        bounds_group.addLayer(layer_tca);
        layerArray.push(layer_tca);
        esfahanCollectorAreaLayer.push(layer_tca);
        CollectorAndDistributertLayer.push(layer_tca);
        map.addLayer(layer_tca);
    }
    //ahvaz
    ...
    //map.createPane('pane_Tabriz_3');
    //map.getPane('pane_Tabriz_3').style.zIndex = 411;
    //map.getPane('pane_Tabriz_3').style['mix-blend-mode'] = 'normal';
    //var layer_Tabriz_3 = new L.geoJson(json_Tabriz_3, {
    //    attribution: '',
    //    interactive: true,
    //    dataVar: 'json_Tabriz_3',
    //    layerName: 'layer_Tabriz_3',
    //    pane: 'pane_Tabriz_3'
    //});
    //bounds_group.addLayer(layer_Tabriz_3);
    //layerArray.push(layer_Tabriz_3);
    //map.addLayer(layer_Tabriz_3);
