...
var newCheckout = function (options) {
    ...
    this.construct = function (options) {
        ...
        $('#btn_SaveNext').click(function () {
            ...
            checkOutModelList[0].IsFromSep = true;
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success == true) {
                        ...
                        //alert('ثبت موفق', 'اطلاعات شما جهت بررسی و ثبت به سامانه ارسال شد. به صفحه مشاهده صورت حساب و پرداخت هدایت خواهید شد', 2);
                        window.location = '/order/SepBillpayment?orderId=' + result.orderIds;
                    }
                    else {
                        alert(result.message);
                        window.location = '/Sep/_Startupt'
                    }
                },
            ...
            onStepChanging: function (vent, currentIndex, newIndex) {
                ...
                if (currentIndex > newIndex) {
                    ...
                    if ($('#tblServiceInfo').find('input[type=radio]:checked').attr('data-IsCod') == 'true') {
                        if (confrim('آیا مبلغ کالای شما باید از گیرنده گرفته شود؟')) {
                            $('#CodGoodsPrice').val($('#ApproximateValue').val());
                        } else { $('#CodGoodsPrice').val(''); };
                    }
                }
                return true;
            },
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.isInvalid == true) {
                        alert('در حال حاضر امکان ثبت سفارش از مبدا مورد نظر وجود ندارد', 2);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($(data).length == 0) {
                        ...
                        alert('با توجه به مقادیر ورودی شما در قسمت وزن،استان و شهرستان فرستنده و گیرنده سرویسی یافت نشد', 3);
                        $('#loader').hide();;
                        canNext = false;
                        return false;
                    }
        ...
        $('#RequestPrintAvatar').click(function () {
            ...
            if ($(this).is(':checked')) {
                ...
                if (avatarStatus == 3)
                    ...
                    alert('کاربر گرامی نشان تجاری شما در نوبت بررسی قرار دارد');
                    ...
                    //        try {
                    //            if (data.errorCode == 0) {
                    //                alert( "بارگذاری با موفقیت انجام شد. بعد از تایید تصویر بارگذاری شده توسط تیم پشتیبانی، می توانید از آن برای چاپ بر روی فاکتور سفارش خود استفاده نمایید");
                    //            }
                    //            else if (data.message) {
                    //                alert( data.message);
                    //            }
                    //            else
                    //                alert('در زمان آپلود فایل خطایی بوجود آمد.لطفا مجددا تلاش کنید');
                    //        }
                    //        catch (ex) {
                    //            alert(err.message);
                    //        }
        ...
        function showAndFillAddress(addressType) {
            if ($('#addressType').val() != 'Sender' && !$('#addressType').attr('showAlert')) {
                ...
                //alert('مکان گیرنده', 'مشخص کردن مکان دقیق گیرنده بر روی نقشه الزامی نیست، مکان را به صورت حدودی(مثلا: منطقه شهری یا شهر، روستاو...) مشخص کنید', '3');
                //closeSidePanel();
            }
